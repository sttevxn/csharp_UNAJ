...
// Una sola lista de empleados que contenga a los encargados. LISTO
	//---> Agregar nuevo atributo booleano a los encargados. LISTO 
	
// Dentro de cada evento hay una lista de servicios.
// Metodo de AGREGAR/ELIMINAR servicio (menu administrativo) tiene que funcionar para cada evento. LISTO
// Las excepciones deben estar pricimalmente en el Program.cs
// Si el cliente cancela con + de un mes de anticipacion -> no se devuelve la seña. // SE USA METODO SUBSTRACT!	LISTO		
// Si el cliente cancela con - de un mes de anticipacion, debe pagar el evento completo. LISTO
...
//PortaPapeles
/*
 public void AgregarServicio(string dniCliente, Servicios nuevoServicio)
		{
			foreach(Eventos eventosContratados in ListaEventosContratados)
			{
				if(eventosContratados.DniCliente==dniCliente)
				{
					eventosContratados.ServiciosContratados.Add(nuevoServicio);
					break;
				}
			}
		}
